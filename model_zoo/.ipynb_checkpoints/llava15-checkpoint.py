            ...
            for i_option in batch["image_options"]:
                ...
                for _ in i_option:
                    ...
                    if method == 'scaling_vis':
                        ...
                        input_token_len = single_input['input_ids'].shape[1]
                        gen = self.processor.decode(output['sequences'][0][input_token_len:], skip_special_tokens=True)
                    ...
                    elif len(list(c_option)) == 6:
                        # Check if the generated text matches the ground truth answer
                        if (answer_list[index_of_total][0] in gen or answer_list[index_of_total][0].lower() in gen.lower()) \
                                and not (answer_list[index_of_total][0].lower() == 'on' and 'front' in gen.strip().lower()):
                            acc += 1
                            correct_id.append(index_of_total)
                            # Mark ground truth at index 0 as correct
                            answers = [1, 0, 0, 0, 0, 0]
                        else:
                            # Mark as incorrect
                            answers = [0, 0, 0, 0, 0, 0]
